...
namespace property_dealer_API.Application.Services.CardManagement
{
    public class CardFactoryService : ICardFactoryService
    {
        ...
        public List<Card> StartCardFactory()
        {
            ...
            var cardDefinitions = JsonSerializer.Deserialize<List<DeckConfiguration>>(
                baseDeckJsonString,
                options
            );
        ...
        private static void GenerateMoneyCards(
            List<Card> deck,
            List<MoneyCardDefinition> moneyCardsToGen
        )
        {
        ...
        private static void GenerateSystemCards(
            List<Card> deck,
            List<SystemCardDefinition> systemCardsToGen
        )
        {
            foreach (var cardsDef in systemCardsToGen)
            {
                if (
                    Enum.TryParse<PropertyCardColoursEnum>(
                        cardsDef.Color,
                        true,
                        out PropertyCardColoursEnum colorEnum
                    )
                )
                {
                    for (int i = 0; i < cardsDef.Count; i++)
                    {
                        var card = new StandardSystemCard(
                            ...
                            cardsDef.Value,
                            colorEnum,
                            cardsDef.Description,
                            cardsDef.MaxCards,
                            cardsDef.RentalValues
                        );
                        deck.Add(card);
                    }
                }
                ...
                {
                    Console.WriteLine(
                        $"Warning: Invalid color '{cardsDef.Color}' found for card '{cardsDef.Name}'. This card will not be generated."
                    );
                }
            }
        }

        private static void GenerateSystemWildCards(
            List<Card> deck,
            List<SystemWildCardDefinition> systemWildCardsToGen
        )
        {
            foreach (var cardsDef in systemWildCardsToGen)
            {
                for (int i = 0; i < cardsDef.Count; i++)
                {
                    var card = new SystemWildCard(
                        CardTypesEnum.SystemWildCard,
                        cardsDef.Name,
                        cardsDef.Value,
                        cardsDef.Description
                    );
        ...
        private static void GenerateCommandCards(
            List<Card> deck,
            List<CommandCardDefinition> commandCardsToGen
        )
        {
            foreach (var cardsDef in commandCardsToGen)
            {
                ...
                {
                    Console.WriteLine(
                        $"Warning: Invalid ActionTypes '{cardsDef.Type}' found for card '{cardsDef.Name}'. This card will not be generated."
                    );
                }
            }
        }

        private static void GenerateTributeCards(
            List<Card> deck,
            List<TributeCardDefinition> tributeCardsToGen
        )
        {
            foreach (var cardsDef in tributeCardsToGen)
            {
                ...
                foreach (var colors in colorsList)
                {
                    if (
                        Enum.TryParse<PropertyCardColoursEnum>(
                            colors,
                            true,
                            out PropertyCardColoursEnum colorsEnum
                        )
                    )
                    {
                    ...
                    {
                        Console.WriteLine(
                            $"Warning: Invalid color '{colors}' found for rental card. This card will not be generated."
                        );
                        continue;
                    }
                }
                ...
                for (int i = 0; i < cardsDef.Count; i++)
                {
                    var card = new TributeCard(
                        CardTypesEnum.TributeCard,
                        cardsDef.Value,
                        colorsEnumAsList,
                        cardsDef.Description
                    );
